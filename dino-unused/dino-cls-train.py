...
import dino.utils as dinoUtils
...
def get_args_parser():
    ...
    parser.add_argument('--norm_last_layer', default=True, type=dinoUtils.bool_flag,
    ...
    parser.add_argument('--use_bn_in_head', default=False, type=dinoUtils.bool_flag,
    ...
    parser.add_argument('--use_fp16', type=dinoUtils.bool_flag, default=True, help="""Whether or not
    ...
    if pargs.load_json:
        with open(pargs.load_json, 'rt') as f:
            pargs.__dict__.update(json.load(f))

    return  pargs


def train_dino(args):
    dinoUtils.init_distributed_mode(args)
    dinoUtils.fix_random_seeds(args.seed)
    print("git:\n  {}\n".format(dinoUtils.get_sha()))
    ...
    sampler = torch.dinoUtils.data.DistributedSampler(dataset, shuffle=True)
    data_loader = torch.dinoUtils.data.DataLoader(
        dataset,
        sampler=sampler,
    ...
    student = dinoUtils.MultiCropWrapper(student, DINOHead(
        embed_dim,
        args.out_dim,
    ...
    if dinoUtils.has_batchnorms(student):
    ...
    lr_schedule = dinoUtils.cosine_scheduler(
        args.lr * (args.batch_size_per_gpu * dinoUtils.get_world_size()) / 256.,  # linear scaling rule
        args.min_lr,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.saveckp_freq and epoch % args.saveckp_freq == 0:
            dinoUtils.save_on_master(save_dict, os.path.join(args.output_dir, f'checkpoint{epoch:04}.pth'))
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                     'epoch': epoch}
        if dinoUtils.is_main_process():
...
class DataAugmentationDINO(object):
    def __init__(self, global_crops_scale, local_crops_scale, local_crops_number):
        ...
        self.global_transfo2 = transforms.Compose([
            transforms.RandomResizedCrop(224, scale=global_crops_scale, interpolation=Image.BICUBIC),
            flip_and_color_jitter,
            dinoUtils.GaussianBlur(0.1),
            dinoUtils.Solarization(0.2),
            normalize,
        ])
